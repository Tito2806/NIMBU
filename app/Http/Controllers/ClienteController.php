...
class ClienteController extends Controller
{
    ...
    public function repositorioFiltro(Request $request)
    {
        return response()->json(['success'=>'Filtro!']);

    }
    public function repositorioFiltroView(Request $request)
    {
        $repositorio = RepositorioArchivo::select('*')->where('titulo', 'LIKE', '%'.$request->filtro.'%')->orwhere('autor', 'LIKE', '%'.$request->filtro.'%')->paginate(6);
        return view('Client.repositorioFiltro', compact('repositorio'));
    }
